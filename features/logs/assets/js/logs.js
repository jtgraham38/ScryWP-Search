document.addEventListener('DOMContentLoaded', function() {
    ...
    var deleteOldLogsButton = document.querySelector('.scrywp-delete-old-logs');
    var deleteOldLogsResult = document.querySelector('.scrywp-delete-old-logs-result');
    ...
    if (deleteOldLogsButton) {
        deleteOldLogsButton.addEventListener('click', function() {
            if (!window.scrywpLogs || !window.scrywpLogs.ajaxUrl || !window.scrywpLogs.deleteOldLogsAction) {
                setDeleteOldLogsResult('Log AJAX settings are missing.');
                return;
            }

            if (!window.confirm(window.scrywpLogs.i18n.deleteOldLogsConfirm || 'Are you sure?')) {
                return;
            }

            deleteOldLogsButton.disabled = true;
            setDeleteOldLogsResult(window.scrywpLogs.i18n.loading);

            var formData = new FormData();
            formData.append('action', window.scrywpLogs.deleteOldLogsAction);
            formData.append('nonce', window.scrywpLogs.deleteOldLogsNonce);
            ...
            fetch(window.scrywpLogs.ajaxUrl, {
                ...
                .then(function(response) {
                    ...
                    var deleted = typeof response.data.deleted === 'number' ? response.data.deleted : 0;

                    if (deleted === 0) {
                        setDeleteOldLogsResult(window.scrywpLogs.i18n.deleteOldLogsNothingToDelete);
                        return;
                    }

                    setDeleteOldLogsResult(window.scrywpLogs.i18n.deleteOldLogsDeleted.replace('%d', String(deleted)));
                })
                .catch(function(error) {
                    setDeleteOldLogsResult(error.message || window.scrywpLogs.i18n.error);
                })
                .finally(function() {
                    deleteOldLogsButton.disabled = false;
                });
        });
    }
    ...
    function setDeleteOldLogsResult(message) {
        if (!deleteOldLogsResult) {
            return;
        }

        deleteOldLogsResult.textContent = message || '';
    }
