...
const {
  ...
  callbacks: {
    init() {
      ...
      const selectedFacetIds = Array.isArray(context?.selectedFacetIds) ? context.selectedFacetIds.map(id => Number(id)) : [];
      if (selectedFacetIds.length > 0 && state.selectedTaxonomyFacets.length === 0) {
        const allTerms = Object.values(context?.facetTaxonomies ?? {}).flat();
        state.selectedTaxonomyFacets = selectedFacetIds.map(id => allTerms.find(term => Number(term.id) === id)).filter(Boolean);
      }
      ...
      if (document.readyState === 'complete') {
        ...
        window.addEventListener('load', bindSearchForms, {
          once: true
        });
      }
    }
